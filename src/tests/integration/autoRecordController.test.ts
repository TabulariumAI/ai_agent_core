...
jest.mock("../../application/useCases/autoRecordUseCase");

describe('AutoRecordController Integration', () => {

    let app: any;
    let useCaseMock: jest.Mocked<AutoRecordUseCase>;

    const url = "/autorecord";

    beforeAll(() => {
        routingUseContainer(Container);

        useCaseMock = new (AutoRecordUseCase as any)();

        Container.set(ChoiceService, new ChoiceService());
        Container.set(FileTypeService, new FileTypeService());
        Container.set(AutoRecordUseCase, useCaseMock);
